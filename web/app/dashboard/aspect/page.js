...
export default function AspectPage() {
  ...
  const fetchHistory = useCallback(async () => {
    try {
      const res = await fetch('/api/telemetry?type=aspect_emotion');
      if (res.ok) {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const res = await fetch('/api/predict', {
        ...
        body: JSON.stringify({ text: text.trim(), type: 'aspect_emotion' }),
      });

      if (!res.ok) {
        const errData = await res.json();
        throw new Error(errData.error || 'Aspect analysis failed');
      }
    ...
    <div className={styles.playground}>
      ...
      <div className={styles.inputSection}>
        <div className={styles.overviewHeader} style={{ borderBottom: 'none', paddingBottom: 0 }}>
          <div>
            <span className={styles.sysTag}>// PIPELINE: ASPECT_BASED_EMOTIONS</span>
            <h1 className={styles.mainTitle} style={{ fontSize: '1.5rem' }}>ASPECT_EMOTION_LAB</h1>
          </div>
        </div>
        ...
        <div className={`${styles.inputCard} cyber-card`} style={{ marginTop: '16px' }}>
          ...
          <textarea
            ...
            placeholder="Type text discussing specific pipeline parts (e.g. database, UI, model, latency, auth)..."
            value={text}
          ...
          <div className={styles.inputActions}>
            <span className={styles.shortcutHint}>[NOUN_TARGETING_RESOLVER]</span>
            <button
              className={styles.analyzeBtn}
              onClick={handleAnalyze}
              disabled={analyzing || !text.trim()}
            >
              {analyzing ? 'MAPPING_ASPECTS...' : 'RUN_ASPECT_MAP'}
            </button>
          </div>
        </div>
        ...
        <div className={styles.samples}>
          ...
          {[
            { text: 'I love how responsive the new UI dashboard is, but the neon database is causing high latency.', emoji: '🏷️' },
            { text: 'The FastAPI training pipeline is amazing and fast, but the authentication key config is confusing.', emoji: '🏷️' },
          ].map((sample, idx) => (
            <button
              key={idx}
      ...
      <div className={styles.resultSection}>
        ...
        {result ? (
          <div className={`${styles.resultCard} cyber-card`} style={{ marginBottom: '24px' }}>
            <span className="corner-brackets" />
            <h3 className={styles.panelTitle} style={{ marginBottom: '16px' }}>// MAPPED_ASPECT_COGNITION:</h3>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {result.metadata?.aspects?.map((item, idx) => (
                <div key={idx} style={{ background: 'rgba(255,255,255,0.01)', border: '1px solid var(--border)', padding: '14px', borderRadius: '4px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                    <span style={{ color: 'var(--accent-cyan)', fontWeight: 'bold', fontFamily: 'var(--font-mono)', fontSize: '0.75rem' }}>
                      ⚡ {item.aspect}
                    </span>
                    <span style={{ fontSize: '0.75rem', fontWeight: 'bold', color: 'var(--accent-indigo)' }}>
                      {EMOJI_MAP[item.emotion] || '😐'} {item.emotion.toUpperCase()} ({Math.round(item.confidence * 100)}%)
                    </span>
                  </div>
                  ...
                  <div className={styles.confidenceBarWrap}>
                    <div
                      ...
                      style={{ width: `${Math.round(item.confidence * 100)}%`, background: 'var(--accent-indigo)' }}
                    />
                  </div>
                </div>
              ))}
            ...
            <div style={{ marginTop: '24px', borderTop: '1px dashed var(--border)', paddingTop: '16px' }}>
              {feedbackSubmitted ? (
                <span style={{ color: 'var(--accent-cyan)', fontSize: '0.75rem', fontFamily: 'var(--font-mono)' }}>
                  ✓ [SYS] ASPECT_ALIGNMENT_TELEMETRY_SYNCED.
                </span>
              ) : (
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                  ...
                  >
                    ...
                    <option value="1 aspects mapped">1 ASPECT</option>
                    <option value="2 aspects mapped">2 ASPECTS</option>
                    <option value="3 aspects mapped">3 ASPECTS</option>
                  </select>
                  <button
          ...
          !error && (
            <div className={`${styles.emptyResult} cyber-card`} style={{ marginBottom: '24px' }}>
              ...
              <h3>ASPECTS_IDLE</h3>
              <p>Enter a sequence about system components to resolve targets and map cognitive values.</p>
            </div>
          )
        )}
        ...
        <div className={`${styles.panelCard} cyber-card`}>
          <span className="corner-brackets" />
          <h2 className={styles.panelTitle} style={{ marginBottom: '16px' }}>// AUDIT_LOG: ASPECT_EMOTION</h2>
          {loadingHistory ? (
              ...
              <table className={styles.table} style={{ width: '100%', fontSize: '0.75rem' }}>
                <thead>
                  <tr style={{ textAlign: 'left', borderBottom: '1px solid var(--border)' }}>
                    ...
                    <th style={{ padding: '8px' }}>MAPPED_COUNT</th>
